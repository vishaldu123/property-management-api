...
class MaintenanceRepository {
    ...
    async create(organizationId, data, userId) {
        return prisma_1.default.maintenanceRequest.create({
            data: {
                organizationId,
    ...
    async findByIdAndOrganizationId(id, organizationId) {
        return prisma_1.default.maintenanceRequest.findFirst({
            where: {
                id,
                organizationId,
                deletedAt: null,
    ...
    async listWithFilters(organizationId, options, filter, search) {
        ...
        if (filter) {
            ...
            if (filter.propertyId)
                whereClause.propertyId = filter.propertyId;
            if (filter.unitId)
        ...
        const [requests, total] = await Promise.all([
            prisma_1.default.maintenanceRequest.findMany({
                where: whereClause,
                orderBy,
                skip,
                take,
            }),
            prisma_1.default.maintenanceRequest.count({
                where: whereClause,
            }),
        ]);
    ...
    async update(id, organizationId, data, userId) {
        ...
        if (data.startedDate !== undefined)
            updateData.startedDate = data.startedDate ? new Date(data.startedDate) : null;
        ...
        return prisma_1.default.maintenanceRequest.update({
            where: {
                id,
                organizationId,
                deletedAt: null,
     ...
     */
    async changeStatus(id, organizationId, status, userId) {
        const updateData = {
            status,
            updatedBy: userId,
        };
     ...
     */
    async findByPropertyId(organizationId, propertyId, options) {
        const skip = options ? ((options.page || 1) - 1) * (options.limit || 20) : 0;
        const take = options?.limit || 20;
        return prisma_1.default.maintenanceRequest.findMany({
            where: {
                organizationId,
                propertyId,
                deletedAt: null,
     ...
     */
    async findByUnitId(organizationId, unitId) {
        return prisma_1.default.maintenanceRequest.findMany({
            where: {
                organizationId,
                unitId,
    ...
    async getOrganizationStatistics(organizationId) {
        ...
        const totalEstimatedCost = requests.reduce((sum, r) => sum + (r.estimatedCost?.toNumber() || 0), 0);
        const totalActualCost = requests.reduce((sum, r) => sum + (r.actualCost?.toNumber() || 0), 0);
        return {
            totalRequests,
            byStatus,
            byPriority,
