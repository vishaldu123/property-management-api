...
describe('Tenant Domain E2E Tests', () => {
    ...
    beforeAll(async () => {
        ...
        // Create unit
        const unitRes = await (0, supertest_1.default)(app_1.default)
            .post('/api/v1/units')
            .set('Authorization', `Bearer ${authToken}`)
            .send({
            propertyId,
    ...
    afterAll(async () => {
        // Cleanup
        await prisma_1.default.tenant.deleteMany({
            where: { organizationId },
        });
        await prisma_1.default.unit.deleteMany({
            where: { propertyId },
        });
        await prisma_1.default.property.deleteMany({
            where: { organizationId },
        });
    });
    ...
    describe('UPDATE Tenant', () => {
        ...
        it('should reject duplicate email on update', async () => {
            ...
            // Create two tenants
            const tenant1Res = await (0, supertest_1.default)(app_1.default)
                .post('/api/v1/tenants')
                .set('Authorization', `Bearer ${authToken}`)
                .send({
                firstName: 'Tenant1',
            ...
            const tenant2Res = await (0, supertest_1.default)(app_1.default)
                .post('/api/v1/tenants')
                .set('Authorization', `Bearer ${authToken}`)
                .send({
                firstName: 'Tenant2',
    ...
    describe('LIST Tenants', () => {
        ...
        it('should search tenants by first name', async () => {
            ...
            if (res.body.data.length > 0) {
                res.body.data.forEach((tenant) => {
                    expect(tenant.firstName.toLowerCase().includes('johnny') ||
                        tenant.lastName.toLowerCase().includes('johnny') ||
                        tenant.email.toLowerCase().includes('johnny')).toBe(true);
                });
            }
        });
    ...
    describe('Organization Isolation', () => {
        ...
        it('should not list other organization tenants', async () => {
            ...
            const hasOtherOrgTenant = res.body.data.some((t) => t.id === otherTenantId);
