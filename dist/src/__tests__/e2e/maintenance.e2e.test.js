...
describe('Maintenance Domain E2E Tests', () => {
    ...
    let organizationId = '';
    let propertyId = '';
    const buildMaintenancePayload = (overrides = {}) => ({
        propertyId,
    ...
    beforeAll(async () => {
        ...
        userId = (0, auth_helpers_1.getUserId)(registerRes.body);
    ...
    afterAll(async () => {
        if (organizationId) {
            await prisma_1.default.maintenanceRequest.deleteMany({ where: { organizationId } });
            await prisma_1.default.property.deleteMany({ where: { organizationId } });
            await prisma_1.default.organizationUser.deleteMany({ where: { organizationId } });
            await prisma_1.default.organization.delete({ where: { id: organizationId } }).catch(() => undefined);
        }
    ...
    describe('Complete maintenance workflow', () => {
        ...
        it('should soft delete and restore a maintenance request', async () => {
            ...
            const listAfterDelete = await (0, supertest_1.default)(app_1.default)
                ...
                .set('Authorization', `Bearer ${authToken}`)
                .expect(200);
            const deletedItems = listAfterDelete.body.data.data;
            expect(deletedItems.find((item) => item.id === maintenanceId)).toBeUndefined();
            const restoreRes = await (0, supertest_1.default)(app_1.default)
                .patch(`/api/v1/maintenance/${maintenanceId}/restore`)
                .set('Authorization', `Bearer ${authToken}`)
                .expect(200);
            expect(restoreRes.body.data.deletedAt).toBeNull();
            const listAfterRestore = await (0, supertest_1.default)(app_1.default)
                .get('/api/v1/maintenance')
                .set('Authorization', `Bearer ${authToken}`)
                .expect(200);
            const restoredItems = listAfterRestore.body.data.data;
        ...
        it('should filter maintenance requests by status, priority, and category', async () => {
            ...
            await (0, supertest_1.default)(app_1.default)
                ...
                .expect(201);
            const byStatus = await (0, supertest_1.default)(app_1.default)
                .get('/api/v1/maintenance?status=Open')
                .set('Authorization', `Bearer ${authToken}`)
                .expect(200);
            expect(byStatus.body.data.data.length).toBeGreaterThan(0);
            expect(byStatus.body.data.data.every((item) => item.status === 'Open')).toBe(true);
            const byPriority = await (0, supertest_1.default)(app_1.default)
                .get('/api/v1/maintenance?priority=High')
                .set('Authorization', `Bearer ${authToken}`)
                .expect(200);
            expect(byPriority.body.data.data.every((item) => item.priority === 'High')).toBe(true);
            const byCategory = await (0, supertest_1.default)(app_1.default)
                .get('/api/v1/maintenance?category=Plumbing')
                .set('Authorization', `Bearer ${authToken}`)
                .expect(200);
            expect(byCategory.body.data.data.every((item) => item.category === 'Plumbing')).toBe(true);
        });
        it('should search maintenance requests by request number and title', async () => {
            const createRes = await (0, supertest_1.default)(app_1.default)
                ...
                .expect(201);
            const maintenanceId = createRes.body.data.id;
            const searchByNumber = await (0, supertest_1.default)(app_1.default)
                .get(`/api/v1/maintenance?search=MR-SEARCH-${seed}`)
                .set('Authorization', `Bearer ${authToken}`)
                .expect(200);
            expect(searchByNumber.body.data.data.find((item) => item.id === maintenanceId)).toBeDefined();
            const searchByTitle = await (0, supertest_1.default)(app_1.default)
                .get(`/api/v1/maintenance?search=${encodeURIComponent(`Unique Title ${seed}`)}`)
                .set('Authorization', `Bearer ${authToken}`)
                .expect(200);
            expect(searchByTitle.body.data.data.find((item) => item.id === maintenanceId)).toBeDefined();
        });
        it('should sort maintenance requests by different fields', async () => {
            ...
            await (0, supertest_1.default)(app_1.default)
                ...
                .expect(201);
            const sortedDesc = await (0, supertest_1.default)(app_1.default)
                .get('/api/v1/maintenance?sortBy=priority&sortOrder=desc')
                .set('Authorization', `Bearer ${authToken}`)
                .expect(200);
            expect(sortedDesc.body.data.data.length).toBeGreaterThan(0);
            const sortedAsc = await (0, supertest_1.default)(app_1.default)
        ...
        it('should enforce organization isolation', async () => {
            const createRes = await (0, supertest_1.default)(app_1.default)
                ...
                .expect(201);
            const maintenanceId = createRes.body.data.id;
            const otherRegisterRes = await (0, supertest_1.default)(app_1.default)
                .post('/api/v1/auth/register')
                .send({
                ...
                .expect(201);
            const otherToken = (0, auth_helpers_1.getAccessToken)(otherRegisterRes.body);
            await (0, supertest_1.default)(app_1.default)
                .get(`/api/v1/maintenance/${maintenanceId}`)
        ...
        it('should get organization statistics', async () => {
            ...
            await (0, supertest_1.default)(app_1.default)
                ...
                .expect(201);
            const statsRes = await (0, supertest_1.default)(app_1.default)
